...
export type TimelineData = {
  ...
  quarters: {
    months: {
      days: number;
      monthIndex: number; // Actual month number (0-11)
    }[];
  }[];
}[];
...
export const createTimelineDataForRange = (startDate: Date, endDate: Date) => {
  const data: TimelineData = [];
  ...
  // Create data only for years/months in the actual range
  for (let year = startYear; year <= endYear; year++) {
    const yearData = {
      year,
      quarters: [] as { months: { days: number; monthIndex: number }[] }[],
    };
    ...
    // Group months into quarters, but only include relevant months
    const quarters = [
      { months: [] as { days: number; monthIndex: number }[] }, // Q1: Jan-Mar
      { months: [] as { days: number; monthIndex: number }[] }, // Q2: Apr-Jun
      { months: [] as { days: number; monthIndex: number }[] }, // Q3: Jul-Sep
      { months: [] as { days: number; monthIndex: number }[] }, // Q4: Oct-Dec
    ];

    for (let month = firstMonth; month <= lastMonth; month++) {
      const quarterIndex = Math.floor(month / 3);
      quarters[quarterIndex].months.push({
        days: getDaysInMonth(new Date(year, month, 1)),
        monthIndex: month,
      });
    }

    // Only add quarters that have months
    yearData.quarters = quarters.filter(q => q.months.length > 0);

    // Only add year if it has quarters
        ...
        <div className="relative flex flex-col" key={`${year.year}-${month.monthIndex}`}>
          <GanttContentHeader
            ...
            renderHeaderItem={(item: number) => (
              <div className="flex items-center justify-center gap-1">
                <p>
                  {format(addDays(new Date(year.year, month.monthIndex, 1), item), 'd')}
                </p>
                <p className="text-muted-foreground">
                  {format(
                    addDays(new Date(year.year, month.monthIndex, 1), item),
                    'EEEEE'
                  )}
                </p>
              </div>
          ...
          <GanttColumns
            ...
            isColumnSecondary={(item: number) =>
              [0, 6].includes(
                addDays(new Date(year.year, month.monthIndex, 1), item).getDay()
              )
            }
          />
        </div>
...
const MonthlyHeader: FC = () => {
  ...
  return gantt.timelineData.map((year) => {
    const months = year.quarters.flatMap((quarter) => quarter.months);
    return (
      <div className="relative flex flex-col" key={year.year}>
        <GanttContentHeader
          columns={months.length}
          renderHeaderItem={(item: number) => (
            <p>{format(new Date(year.year, months[item].monthIndex, 1), 'MMM')}</p>
          )}
          title={`${year.year}`}
        />
...
const QuarterlyHeader: FC = () => {
  ...
  return gantt.timelineData.map((year) =>
    year.quarters.map((quarter, quarterIndex) => {
      const firstMonthIndex = quarter.months[0]?.monthIndex ?? 0;
      const actualQuarter = Math.floor(firstMonthIndex / 3) + 1;
      return (
        <div
        ...
        >
          <GanttContentHeader
            ...
            renderHeaderItem={(item: number) => (
              <p>
                {format(new Date(year.year, quarter.months[item].monthIndex, 1), 'MMM')}
              </p>
            )}
            title={`Q${actualQuarter} ${year.year}`}
          />
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const [timelineData, setTimelineData] = useState<TimelineData>(
    initialTimelineData || createInitialTimelineData(new Date())
  );
  const [, setScrollX] = useGanttScrollX();
  const [sidebarWidth, setSidebarWidth] = useState(0);
  const isCustomTimeline = Boolean(initialTimelineData);
  ...
  useEffect(() => {
    if (scrollRef.current) {
      // For custom timelines, start at the beginning
      // For default timelines, center on today
      if (isCustomTimeline) {
  ...
  const handleScroll = useCallback(
    throttle(() => {
      ...
      // Skip auto-extension if using custom timeline data
      if (isCustomTimeline) {
        return;
      }
